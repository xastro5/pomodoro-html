...
- **Weather Widget**: Uses the earlier full-color emoji by default; click the weather icon to switch between emoji and colored outlines. Click the temperature to switch between ℃ and ℉. These choices are never saved: each new opening or refresh starts with emoji and ℃. Unavailable readings are hidden
...
- **Finish Time**: A running timer shows its planned finish time; a paused timer shows Paused. The countdown uses an end timestamp, so delayed browser callbacks do not accumulate drift. Pause preserves the remaining time. A completed session stays at 00:00 until its glow is acknowledged.
...
When time is up, a halo and window-edge glow pulse together every 2 seconds for the first 10 seconds, then every 3 seconds until a new click anywhere in the app or a key press acknowledges them. The clicked control still performs its normal action. Focus completion uses red, break completion uses green, and alarms use amber around the alarm card and window. Acknowledging the visuals does not silence an alarm: use Snooze or Dismiss (or Escape) as usual. The timer keeps the completed phase, its color, 00:00, and its completion message until acknowledgment. It then readies the next phase without starting it automatically. Clicking Start acknowledges and starts that next phase; Reset leaves it at its full duration, duration controls edit it, and choosing a preset selects that preset. The cues work with sound off, stay steady until interaction with reduced motion, and wait for the page to become visible if they start in a background tab. They never save state between openings.
